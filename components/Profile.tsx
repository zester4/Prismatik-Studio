...
export default function Profile(): ReactElement {
  ...
  const { stats, creativeStreak } = useMemo(() => {
    // FIX: Removed overly broad Record<string, number> type to allow TypeScript to infer the specific shape of the object.
    const counts = { image: 0, video: 0, story: 0, logo: 0, ad: 0 };
    ...
    if (historyItems.length > 0) {
        ...
        const uniqueDays = [...new Set(historyItems.map(item => new Date(item.timestamp).setHours(0, 0, 0, 0)))].sort((a, b) => b - a);
